...
with zipfile.ZipFile(root/'Balance_Simulation_Bundle.zip','w',zipfile.ZIP_DEFLATED) as z:
    ...
    for name in ('construction_rules.py','construction_tests.py','battle_setup.py','battle_setup_tests.py','sim_replay.py','sim_replay_tests.py','sim_scenarios.py','sim_scenario_tests.py','mechanics_smoke.py','Simulation_Mechanics_Coverage.md'):z.write(root/name,name)
    for p in sorted((root/'shared-results-v2').glob('*.json')):z.write(p,'shared-results-v2/'+p.name)
    for folder in ('planner-results-20260916','planner-results-20260916-ruling','mechanics-diagnostics-20260916'):
